...
void print_board(struct board *b)
{
	...
	for (i = MIN_NUM; i <= MAX_NUM; ++i) {
		...
		for (j = MIN_NUM; j <= MAX_NUM; ++j){
			...
			if(fmod(j, sqrt(MAX_NUM)) == 0) printf("|");
		}
		if(fmod(i, sqrt(MAX_NUM)) == 0) printf("\n          ---+---+----");
		printf("\n");
...
int solve_board(struct board *b, int r, int c)
{
	...
	if (b->unset_cells == 0) {
		//printf("\n\nOne solution of the Sudoku problem is: \n\n\n");
		//print_board(b);
		return 1;
	}
	...
	while (1) {
		...
		if (solve_board(b, r, c)){
			//printf("\n\n");
			sol_num ++;
			printf("The solution %d of the problem is:\n", sol_num);
			print_board(b);
		}
...
int main(int argc, char *argv[])
{
	...
	read_board(in, &b);
	printf("\n\nThe input of the sudoku problem is: \n\n\n");
	print_board(&b);
